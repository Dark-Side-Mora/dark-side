...
export class GithubAppService {
  ...
  /**
   * Step 1: Create a pending connection record when user initiates GitHub connection
   * This tracks the connection attempt so we can sync repos when user returns
   */
  async createPendingConnection(
    ...
    try {
      ...
      await this.prisma.gitHubAppConnectionPending.upsert({
        where: {
      ...
      console.log(
        `[GithubAppService] Created pending connection for user ${userId} in org ${organizationId}`,
      );
    } catch (error) {
      console.error(
        `[GithubAppService] Error creating pending connection: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  /**
   * Step 2 & 3: Sync repositories for organization
   * 1. Check for pending connection (tracking record)
   * 2. Get existing projects from database for this organization
   * 3. Get repositories from GitHub for this user
   * 4. Compare and update:
   *    - If DB has repos not in GitHub: user removed them → delete from DB
   *    - If GitHub has new repos: add them to organization
  ...
  async syncRepositoriesForOrganization(
    ...
    try {
      console.log(
        `[GithubAppService] Starting sync for org ${organizationId} (user: ${userId})`,
      );

      // Step 1: Check for pending connection (tracking record)
      const pendingConnection =
        await this.prisma.gitHubAppConnectionPending.findUnique({
          where: {
            userId_organizationId: {
              userId,
              organizationId,
      ...
      // Step 2: Get existing projects/repos from database for this organization
      ...
      console.log(
        `[GithubAppService] Found ${existingProjects.length} existing GitHub projects for org ${organizationId}`,
      );

      // Create a set of existing repo IDs in THIS organization only
      const existingRepoIds = new Set(
        existingProjects
          .flatMap((p) => p.githubRepositories)
          .map((r) => r.repositoryId),
      );

      // Step 3: Get all GitHub installations and repos for this user
      const userInstallations =
        await this.prisma.gitHubInstallationUser.findMany({
          where: { userId },
          include: {
            installation: {
              include: {
      ...
      console.log(
        `[GithubAppService] Found ${userInstallations.length} GitHub installations for user`,
      );

      let totalNewRepos = 0;
      let totalRemovedRepos = 0;

      // Collect all GitHub repo IDs from all installations
      const allGitHubRepoIds = new Set<string>();
      const allGitHubRepos: Array<{ repo: any; installationId: string }> = [];

      // Process each installation to get current GitHub repos
      for (const userInstallation of userInstallations) {
        const installation = userInstallation.installation;

        try {
          console.log(
            `[GithubAppService] Processing installation ${installation.installationId}`,
          );

          // Get actual repos from GitHub API
          const gitHubRepos = await this.fetchRepositoriesForInstallation(
            installation.installationId,
          );

          console.log(
            `[GithubAppService] Installation ${installation.installationId}: ${gitHubRepos.length} repos from GitHub`,
          );
          // Collect repo IDs and details
          gitHubRepos.forEach((repo) => {
            allGitHubRepoIds.add(repo.id.toString());
            allGitHubRepos.push({
              repo,
          ...
          if (error?.status === 401 || error?.response?.status === 401) {
            console.warn(
              `[GithubAppService] Installation ${installation.installationId} has invalid credentials (401). User needs to reconnect GitHub.`,
            );
            continue;
          }

          console.error(
            `[GithubAppService] Error fetching repos from installation ${installation.installationId}: ${error.message}`,
          );
      ...
      console.log(
        `[GithubAppService] Total unique repos across all installations: ${allGitHubRepoIds.size}`,
      );

      const toAdd = new Set<string>();
      const toDelete = new Set<string>();

      // Compare only repos in THIS organization
      for (const repoId of existingRepoIds) {
        if (!allGitHubRepoIds.has(repoId)) {
          toDelete.add(repoId);
        }
      }
      for (const repoId of allGitHubRepoIds) {
        if (!existingRepoIds.has(repoId)) {
          toAdd.add(repoId);
        }
      }

      // filter add further by removing all

      // Delete - only from THIS organization
      for (const repoId of toDelete) {
        // Find all repo records for this repository ID in this organization
        const githubRepos = await this.prisma.gitHubRepository.findMany({
          where: {
            repositoryId: repoId,
            project: {
              organizationId,
      ...
      // Delete orphaned projects (projects with no GitHub repositories)
      const orphanedProjects = await this.prisma.project.findMany({
        where: {
          organizationId,
          provider: 'github',
          githubRepositories: {
      ...
      for (const orphanedProject of orphanedProjects) {
        await this.prisma.project.delete({
          where: { id: orphanedProject.id },
        });
      }

      // Add new repos if pending connection exists
      if (pendingConnection) {
        console.log(
          `[GithubAppService] Found pending connection for user ${userId} in org ${organizationId}`,
        );

        for (const repoId of toAdd) {
          const ghRepo = allGitHubRepos.find(
            (r) => r.repo.id.toString() === repoId,
          );
          const installation = userInstallations.find(
            (inst) => inst.installation.id === ghRepo?.installationId,
          );
          ...
          let project = await this.prisma.project.findFirst({
            where: {
              organizationId,
              repositoryUrl: ghRepo?.repo.full_name,
            },
          });
          ...
          if (!project) {
            project = await this.prisma.project.create({
              data: {
                name: ghRepo?.repo.name || 'Unknown Repo',
                provider: 'github',
                repositoryUrl: ghRepo?.repo.full_name || 'Unknown URL',
                organizationId,
                userId,
              },
            });
          ...
          // Create GitHub repository tracking record
          await this.prisma.gitHubRepository.upsert({
            where: {
              installationId_repositoryId: {
                installationId: installation?.installation.id || 'Unknown',
                repositoryId: repoId,
              },
            },
            update: {
            ...
            create: {
              repositoryId: repoId,
              name: ghRepo?.repo.name || 'Unknown Repo',
              fullName: ghRepo?.repo.full_name || 'Unknown URL',
              private: ghRepo?.repo.private,
              installationId:
                installation?.installation.id || 'Unknown Installation',
              projectId: project.id,
            },
          });
          totalNewRepos++;
        ...
        // Step 5: Clear the pending connection record after successful sync
        await this.prisma.gitHubAppConnectionPending.delete({
          where: {
        ...
        console.log(
          `[GithubAppService] Cleared pending connection for user ${userId} in org ${organizationId}`,
        );
      } else {
        console.log(
          `[GithubAppService] No pending connection found - skipping add operations`,
        );
      }
      ...
      const summary = {
        ...
        totalNewRepos,
        totalRemovedRepos,
        message: `Synced repositories: added ${totalNewRepos}, removed ${totalRemovedRepos}.`,
      };

      console.log(
        `[GithubAppService] Sync complete: ${JSON.stringify(summary)}`,
      );

      return summary;
    } catch (error) {
      console.error(
        `[GithubAppService] Error in syncRepositoriesForOrganization: ${error.message}`,
      );
      console.error(error);
      throw error;
    }
   ...
   */
  async fetchRepositoriesForInstallation(
    installationId: string,
  ): Promise<GitHubRepository[]> {
    try {
      ...
      console.error(
        `[GithubAppService] Error fetching repositories for installation ${installationId}: ${error.message}`,
      );
      throw error;
    }
  }
